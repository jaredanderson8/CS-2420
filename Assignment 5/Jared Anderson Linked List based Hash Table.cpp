...
template <typename T>
HashTable<T>::HashTable(int _buckets)
  {
    buckets = _buckets;
...
void HashTable<T>::create(const string& key,const T& value)
{
  ...
  pairObject.second = value;

  auto index = hash<string>{}(key) % buckets;
  arr[index].push_front(pairObject);
}


template <typename T>
void HashTable<T>::create(const string& key, T&& value)
{
  auto index = hash<string>{}(key) % buckets;
  arr[index].emplace_front(key, std::move(value));
}
...
template <typename T>
void HashTable<T>::update(string key, T value)
{
  auto index = hash<string>{}(key) % buckets;
...
void HashTable<T>::remove(string key)
{
  ...
  arr[index].remove_if([key](auto& keyVal){
    if(keyVal.first == key)
    {
      return true;
    }
